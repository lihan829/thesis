...
%              & \textit{ço} + \textit{ŋ} \\
% MH          &   & LH         &
%              & M \hspace{1.4em} H && \text{`small fish'}\\
% \end{array}
...
\citet{jardine2017LocalNature,jardine2016locality} first adopted a formal theory
to analyze tonotactics. He proposed that by using model theory to point out that
phonotactic grammar of tones can be generalized as a set of forbidden subgraphs.
Such way of using conjunctive negative literals, or learning from the psotiive 
limit is effeicient. \citet{li2025learning} implemented a bottom-up learning 
algorithm over autosegmental representation that learned hausa grammar over
a  corpus of orthpgrpahyic.


\citep{jardineComputationallyToneDifferent2016} argues that tonal phonology
is different from segmental phonology also in the computational sense for 
many common processes seen in tonal phonology, such as bounded or unbounded
tonal spreading or shift, cannot be learned using deterministric finite
state machines. They are not weakly determinisitc, which constrasts with 
previous typological work on segmental phonology.

\citet{chandlee2019autosegmental}\todo{chandlee glossa paper} lists
that many tonal processes that are not local over a string representation
can acutally modeled in a local way using autosemgnetal representations.

\todo{maxent}
95\% are local by Chandless from p-base